...
- 휴리스틱 벽 그래프와 브라우저 ONNX AI를 결합한 벽·여닫이문·미닫이문·기타문·창 후보 자동 인식
...
- ONNX Runtime Web — 브라우저 내 STR 전이학습 모델 추론
- Lucide React — 아이콘
...
1. 원본에 가까운 평면도를 업로드합니다. 짧은 변 500px 이상을 권장하며, 지나치게 작거나 저대비인 이미지는 안내에 따라 다시 첨부합니다.
...
├── hooks/                    # 이미지 업로드 처리
├── lib/
│   ├── floorPlanDetection.ts # 휴리스틱 벽 + ONNX 문·창 하이브리드 인식
...
├── legacy/
│   └── floorPlanDetectionHeuristic.ts # 이전 휴리스틱-only 인식기
...
- 자동 인식은 휴리스틱 벽과 AI 문·창 후보를 결합하므로 선명한 도면에서도 결과 검토와 수동 보정이 필요합니다. 베란다문은 수동으로 추가합니다.
- 첫 자동 인식 때 ONNX 모델과 WASM 런타임 약 24MB를 내려받습니다. 업로드 이미지는 서버로 전송되지 않고 현재 브라우저에서만 처리됩니다.
- 최소 320×480px 또는 15만 화소보다 작은 이미지와 선 대비가 부족한 이미지는 업로드 단계에서 차단됩니다. 선명한 흑백 선화는 통과할 수 있지만 컬러 원본을 권장합니다.
